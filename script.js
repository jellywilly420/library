...
const requiredTitle = document.querySelector(
  ".title-container .required-title"
);
const requiredAuthor = document.querySelector(
  ".author-container .required-author"
);
  ...
  this.addToLibrary = function () {
    ...
    changeProgressButton.addEventListener("click", () => {
      ...
      for (const progressNode of main.querySelectorAll("p.progress-node")) {
        if (
          progressNode.parentElement.getAttribute("data-title") === this.title
        ) {
          progressNode.innerText = this.readingProgress;
        }
      }
...
addButton.addEventListener("click", (event) => {
  ...
  if (authorInput.value === "" && titleInput.value === "") {
    ...
    if (bookInLibrary) {
      alert(
        `${titleInput.value} by ${authorInput.value} is already in the library.`
      );
      clearValues();
    } else {
